...
async function parseReviewsFromDom(page) {
  ...
  for (let i = 0; i < count && list.length < REVIEW_LIMIT * 3; i++) {
    ...
    const author = cleanText(
      await c.locator('[itemprop="author"], [class*="author"], strong, a')
        .first().textContent().catch(() => null)
    )

    let body = cleanText(
      await c.locator(
        '[itemprop="reviewBody"], [data-testid*="reviewBody"], [class*="reviewBody"], [class*="content"], [class*="text"], p'
      ).first().textContent().catch(() => null)
    )
    ...
    const title = cleanText(
      await c.locator('[itemprop="name"], [class*="title"], h3, h4')
        .first().textContent().catch(() => null)
    )

    const ratingLabel = await c.locator('[aria-label*="별"], [aria-label*="out of 5"]')
...
async function run(url) {
  ...
  page.on('response', async (res) => {
    try {
      ...
      if (!u.includes('review') && !u.includes('ugc') && !u.includes('rating')) return
  ...
  // 리뷰 탭 클릭
  const reviewTab = page.locator('button:has-text("리뷰"), a:has-text("리뷰"), a:has-text("고객 리뷰")')
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const moreBtn = page.locator('button:has-text("더 보기"), button:has-text("Load More"), button:has-text("더보기")')
